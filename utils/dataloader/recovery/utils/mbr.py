from utils.spatial_func import distance, SPoint
...
class MBR:
    ...
    def get_h(self):
        return distance(
            SPoint(self.min_lat, self.min_lng), SPoint(self.max_lat, self.min_lng)
        )

    def get_w(self):
        return distance(
            SPoint(self.min_lat, self.min_lng), SPoint(self.min_lat, self.max_lng)
        )
    ...
    def load_mbr(file_path):
        with open(file_path, "r") as f:
            ...
            mbr = MBR(
                float(attrs[1]), float(attrs[2]), float(attrs[3]), float(attrs[4])
            )
        return mbr
    ...
    def store_mbr(mbr, file_path):
        with open(file_path, "w") as f:
            ...
            f.write(
                "{};{};{};{};{};{}\n".format(
                    0, mbr.min_lat, mbr.min_lng, mbr.max_lat, mbr.max_lng, mbr.to_wkt()
                )
            )
