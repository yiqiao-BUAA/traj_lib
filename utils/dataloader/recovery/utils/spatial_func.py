...
def distance(a, b):
    ...
    h = math.sin(delta_lat / 2.0) * math.sin(delta_lat / 2.0) + math.cos(
        math.radians(a.lat)
    ) * math.cos(math.radians(b.lat)) * math.sin(delta_lng / 2.0) * math.sin(
        delta_lng / 2.0
    )
...
def bearing(a, b):
    ...
    x = math.cos(pt_a_lat_rad) * math.sin(pt_b_lat_rad) - math.sin(
        pt_a_lat_rad
    ) * math.cos(pt_b_lat_rad) * math.cos(pt_b_lng_rad - pt_a_lng_rad)
...
def project_pt_to_road(rn, t, rid):
    ...
    dis = [
        distance(t, SPoint(edge_cords[2 * i], edge_cords[2 * i + 1]))
    ...
    if idx != 0:
        candidate.append(
            [
                *project_pt_to_segment(
                    SPoint(edge_cords[2 * (idx - 1)], edge_cords[2 * (idx - 1) + 1]),
                    SPoint(edge_cords[2 * idx], edge_cords[2 * idx + 1]),
                    t,
                ),
                idx,
            ]
    ...
    dist_to_end = (1 - rate) * distance(
        SPoint(edge_cords[2 * (idx - 1)], edge_cords[2 * (idx - 1) + 1]),
        SPoint(edge_cords[2 * idx], edge_cords[2 * idx + 1]),
    ) + rn.edgeOffset[rid][idx]
    if rn.edgeDis[rid] > 0:
...
def rate2gps(rn, rid, rate) -> SPoint:
    ...
    for i in range(len(cords) - 1):
        ...
        if dist >= offset:
            if (
                distance(SPoint(*cords[i]), SPoint(*cords[i + 1])) < 1e-6
            ):  # zero segment length
                coor_rate = 0
            else:
                coor_rate = (offset - pre_dist) / distance(
                    SPoint(*cords[i]), SPoint(*cords[i + 1])
                )
            project_pt = cal_loc_along_line(
                SPoint(*cords[i]), SPoint(*cords[i + 1]), coor_rate
            )
            break
