...
class CandidatePoint(SPoint):
    ...
    def __str__(self):
        return "{},{},{},{},{},{}".format(
            self.eid, self.lat, self.lng, self.error, self.offset, self.rate
        )
...
def get_candidates(pt, rn, search_dist):
    ...
    if len(candidate_edges) > 0:
        candi_pt_list = [
            cal_candidate_point(pt, rn, candidate_edge)
            for candidate_edge in candidate_edges
        ]
        # refinement
        candi_pt_list = [
            candi_pt for candi_pt in candi_pt_list if candi_pt.error <= search_dist
        ]
...
def cal_candidate_point(raw_pt, rn, edge):
    ...
    idx, (projection, coor_rate, dist) = min(
        enumerate(candidates), key=lambda x: x[1][2]
    )
    ...
    for i in range(idx):
        offset += distance(
            coords[i], coords[i + 1]
        )  # make the road distance more accurately
    offset += distance(
        coords[idx], projection
    )  # distance of road start position and projected point
    if rn[u][v]["length"] == 0:
        rate = 0
        # print(u, v)
    else:
        rate = (
            offset / rn[u][v]["length"]
        )  # rate of whole road, coor_rate is the rate of coords.
