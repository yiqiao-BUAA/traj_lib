        ...
        with open(input_path, "r") as f:
            ...
            for line in tqdm.tqdm(f.readlines()):
                ...
                if attrs[0][0] == "-":
                    ...
                    rid = int(attrs[3])
                    if is_target:
                        projection, rate, dist = project_pt_to_road(
                            self.rn, SPoint(lat, lng), rid
                        )
                        candi_pt = CandidatePoint(
                            projection.lat,
                            projection.lng,
                            rid,
                            dist,
                            rate * self.rn.edgeDis[rid],
                            rate,
                        )
                        pt = STPoint(
                            lat,
                            lng,
                            create_datetime(str(attrs[0])),
                            {"candi_pt": candi_pt},
                        )
                    else:
                        pt = STPoint(
                            lat, lng, create_datetime(str(attrs[0])), {"candi_pt": None}
                        )
                    # pt contains all the attributes of class STPoint
                    pt_list.append(pt)
            if len(pt_list) > 1:
