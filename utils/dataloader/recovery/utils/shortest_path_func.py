...
class SPSolver:
    ...
    def update_sp_cache(self, rx: int, ry: int) -> list:
        return self.rn.shortestPath(
            rx,
            ry,
            with_route=True,
            max_len=MAX_ERR + self.rn.edgeDis[ry] + self.rn.edgeDis[rx],
        )[1]

    def update_sp_lru(self, rx: int, ry: int) -> list:
        item = self.sp_cache.get((rx, ry))
        if item == [-1]:  # not hit:
            path = self.rn.shortestPath(
                rx,
                ry,
                with_route=True,
                max_len=MAX_ERR + self.rn.edgeDis[ry] + self.rn.edgeDis[rx],
    ...
    def cal_sp_dist(self, x: SPoint, y: SPoint, rx: int, ry: int) -> float:
        ...
        if rx == ry:
            ...
            travel_dis_xy = (1 - rate_x) * self.rn.edgeDis[
                rx
            ] + rate_y * self.rn.edgeDis[ry]
            travel_dis_yx = (1 - rate_y) * self.rn.edgeDis[
                ry
            ] + rate_x * self.rn.edgeDis[rx]
