...
def cal_rn_dis_loss(
    sp_solver, predict_gps, predict_id, target_gps, target_id, rn_flag=True
):
    """
    ...
    for i in range(trg_len):
        ...
        if rn_flag:
            rn_ls_dis.append(
                sp_solver.cal_sp_dist(
                    SPoint(*predict_gps[i]),
                    SPoint(*target_gps[i]),
                    int(predict_id[i]),
                    int(target_id[i]),
                )
...
def cal_id_acc_batch(predict, target, trg_len, rn, inverse_flag=True, reduction="mean"):
    ...
    if reduction == "mean":
        return (
            np.mean(rid_acc_bs),
            np.mean(rid_recall_bs),
            np.mean(rid_precision_bs),
            np.mean(rid_f1_bs),
        )
    elif reduction == "sum":
        return (
            np.sum(rid_acc_bs),
            np.sum(rid_recall_bs),
            np.sum(rid_precision_bs),
            np.sum(rid_f1_bs),
        )
    else:
...
def cal_rn_dis_loss_batch(
    ...
    for i in range(batch_size):
        ...
        mae, rmse, rn_mae, rn_rmse = cal_rn_dis_loss(
            sp_solver,
            tmp_predict_gps,
            tmp_predict_id,
            tmp_target_gps,
