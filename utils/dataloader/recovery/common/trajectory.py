...
class Trajectory:
    ...
    def get_mid_time(self):
        return (
            self.pt_list[0].time + (self.pt_list[-1].time - self.pt_list[0].time) / 2.0
        )
    ...
    def query_trajectory_by_temporal_range(self, start_time, end_time):
        ...
        start_idx = self.binary_search_idx(
            st
        )  # pt_list[start_idx].time <= st < pt_list[start_idx+1].time
        if self.pt_list[start_idx].time < st:
            # then the start_idx is out of the range, we need to increase it
            start_idx += 1
        end_idx = self.binary_search_idx(
            et
        )  # pt_list[end_idx].time <= et < pt_list[end_idx+1].time
    ...
    def binary_search_idx(self, time):
        ...
        while left_idx <= right_idx:
            ...
            if (
                mid_idx < nb_pts - 1
                and self.pt_list[mid_idx].time <= time < self.pt_list[mid_idx + 1].time
            ):
                return mid_idx
    ...
    def query_location_by_timestamp(self, time):
        ...
        if idx == -1 or idx == len(self.pt_list) - 1:
            return None
        if (
            self.pt_list[idx].time == time
            or (self.pt_list[idx + 1].time - self.pt_list[idx].time).total_seconds()
            == 0
        ):
            ...
            dist_traveled = (
                dist_ab
                * (time - self.pt_list[idx].time).total_seconds()
                / (self.pt_list[idx + 1].time - self.pt_list[idx].time).total_seconds()
            )
            return cal_loc_along_line(
                self.pt_list[idx], self.pt_list[idx + 1], dist_traveled / dist_ab
            )
    ...
    def __hash__(self):
        return hash(
            self.oid
            + "_"
            + self.pt_list[0].time.strftime("%Y%m%d%H%M%S")
            + "_"
...
def get_tid(oid, pt_list):
    return (
        oid
        + "_"
        + pt_list[0].time.strftime("%Y%m%d%H%M%S")
        + "_"
        + pt_list[-1].time.strftime("%Y%m%d%H%M%S")
    )
