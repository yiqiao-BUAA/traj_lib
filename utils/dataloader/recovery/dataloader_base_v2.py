...
from utils.dataloader.recovery.dataloader_base import *
import os
import dgl


## post_process_func, parse the trajectory to our form
def parse_traj(rn, traj, mbr, grid_size, time_span, win_size, ds_type, keep_ratio):
    """
    :param rn: road network
    :param traj: traj seq
    :param mbr: Grid loader
    :param grid_size: sequential learning
    :param time_span: time interval
    :param win_size: traj size
    :param keep_ratio: the ratio for reserving trajectories
    :return:
    """
    ...
    for tr in new_trajs:
        ...
        ls_grid_seq, ls_gps_seq, point_features, ls_src_eids, ls_src_timeids, ls_src_rate_ls, rid_features, hours, ttl_t, choose_rid = get_src_seq(
            rn, ds_pt_list, mbr, grid_size, time_span
        )
...
# get the x sequence
def get_src_seq(rn, ds_pt_list, mbr, grid_size, time_span=15):
    hours = []
    ls_grid_seq = []
    ls_gps_seq = []
    ...
    for ds_pt in ds_pt_list:
        ...
        ls_src_eids.append([ds_pt.data['candi_pt'].eid])
        ls_src_timeids.append([int(ds_pt.time.timestamp())])
        rid_features.append(online_features_dict[ds_pt.data['candi_pt'].eid][4:])

    search_distance = get_search_dist(ds_pt_list)
    choose_rid = [candidate for k, ds_pt in enumerate(ds_pt_list) for candidate in
                  get_candidates_eid(ds_pt, rn, search_distance[k])]
    return ls_grid_seq, ls_gps_seq, point_features, ls_src_eids, ls_src_timeids, ls_src_rate_ls, rid_features, hours, ttl_t, choose_rid


def get_search_distance(pt_list):
    """
...
def post_process_func(
    ...
    """
    pre_process_the_trajectory and get the input
    ...
    ## v2 add new baselines for the data
    all_samples = {
        "src_gps_seq": [],
        "src_grid_seq": [],
        "src_pro_seq": [],
    ...
    for src_traj, trg_traj in tqdm(zip(src_trajs, trg_trajs)):
        _, _, _, _, ls_grid_seq_ls, ls_gps_seq_ls, features_ls, point_features_ls, ls_rid_ls, ls_timeid_ls, ls_src_rate, ls_rid_features = parse_traj(
            rn,
            src_traj,
            mbr,
            grid_size,
        ...
        _, mm_gps_seq_ls, mm_eids_ls, mm_rates_ls, _, _, _, _, _, _, _, _ = parse_traj(
        ...
        all_samples['src_point_fea'].extend(point_features_ls)
        all_samples['src_rid'].extend(ls_rid_ls)
        all_samples['src_rate'].extend(ls_src_rate)
        all_samples['src_road_fea'].extend(ls_rid_features)
        all_samples['choose_rid_all'].extend(choose_rid_ls)
    ...
    return (
        ...
        all_samples['src_point_fea'],
        all_samples['src_rid'],
        all_samples['src_rate'],
        all_samples['src_road_fea'],
        all_samples['choose_rid_all'],
...
@register_dataloader(name=f'Recovery_2')
class BaseDataset_v2(Dataset):
    def __init__(
        ...
        self.src_point_fea, self.src_rid, self.src_timeid, self.src_rate, self.src_road_fea, self.choost_rid_all = [],[],[],[],[],[]
