...
dataset_name = "brightkite_sin"
root_path = Path(__file__).resolve().parent.parent.parent.parent
args = ConfigResolver(f"{root_path}/data/brightkite/{dataset_name}.yaml").parse()


def pre_process_func() -> pd.DataFrame:
    df = pd.read_csv(f"{root_path}/data/brightkite/{dataset_name}.csv")
    df = df[["timestamps", "POI_id", "user_id", "lat", "lon"]]
    df.columns = ["timestamps", "POI_id", "user_id", "latitude", "longitude"]
    df["POI_id"] = pd.factorize(df["POI_id"])[0] + 1
    df["user_id"] = pd.factorize(df["user_id"])[0] + 1
    df["timestamps"] = (
        pd.to_datetime(df["timestamps"], utc=True).astype("int64") // 10**9
    )
    df["POI_catid"] = 0
