...
def pre_process_func() -> pd.DataFrame:
    ...
    df = df[["date", "business_id", "user_id", "latitude", "longitude"]]
    df["business_id"] = pd.factorize(df["business_id"])[0] + 1
    df["user_id"] = pd.factorize(df["user_id"])[0] + 1
    df["date"] = (
        pd.to_datetime(df["date"], utc=True, errors="coerce").astype("int64") // 10**9
    )
